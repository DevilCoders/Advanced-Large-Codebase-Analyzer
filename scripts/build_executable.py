...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Bundle into a single-file executable (enabled by default)",
    )
    parser.add_argument(
        "--no-onefile",
        dest="onefile",
        action="store_false",
        help="Disable single-file bundling",
    )
    parser.add_argument(
        "--windowed",
        dest="windowed",
        action="store_true",
        default=True,
        help="Run the application without an attached console window (enabled by default)",
    )
    parser.add_argument(
        "--console",
        dest="windowed",
        action="store_false",
        help="Keep a console window attached",
    )
    parser.add_argument(
        "--icon",
        help="Path to a .ico file to use for the packaged executable",
    )
    parser.add_argument(
        "--clean",
        action="store_true",
...
def run() -> None:
    ...
    command = [pyinstaller, "--name", args.name, "--noconfirm", "--clean"]
