...
```bash
python scripts/build_executable.py --name advanced-analyzer --icon path/to/icon.ico
```

The helper mirrors the typical Windows packaging command:

```powershell
pyinstaller --noconfirm --onefile --windowed --icon "C:\\path\\to\\icon.ico" "D:\\path\\to\\project\\advanced_codebase_analyzer\\app.py"
```

Passing the `--icon` flag wires up the icon automatically, while the
`--windowed` and `--onefile` options are enabled by default. Use
`--console` or `--no-onefile` to opt out. The script requires PyInstaller
(`pip install pyinstaller`) and writes artifacts to `dist/`.
