...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="onefile",
        action="store_false",
        help="Disable single-file bundling for faster incremental builds.",
    )
    parser.add_argument(
        "--windowed",
        action="store_true",
        help="Use the windowed (GUI) subsystem when packaging (suppresses console window)",
    )
    parser.add_argument(
        "--icon",
        type=Path,
        help="Path to an ICO icon file to embed in the generated executable",
    )
    parser.add_argument(
        "--clean",
        action="store_true",
...
def run() -> None:
    ...
    command = [pyinstaller, "--name", args.name, "--noconfirm"]
    if args.onefile:
