...
python scripts/build_executable.py \
  ...
  --windowed \
  --icon "C:/Users/Coder-Laptop/Downloads/thumb-150-325149.ico"
```

This mirrors the Windows command:

```powershell
pyinstaller --noconfirm --onefile --windowed \
  --icon "C:\\Users\\Coder-Laptop\\Downloads\\thumb-150-325149.ico" \
  "D:\\Codex Projects\\advanced_codebase_analyzer\\app.py"
```

Use `--no-onefile` if you prefer the default multi-file layout. The script
requires PyInstaller (`pip install pyinstaller`) and writes artifacts to `dist/`.
